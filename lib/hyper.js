...
/* getUserPerpMargin
 * 
 * This methods relies on you providing the accountId then it 
 * will query the Hyperliquid APIs for the information.
 * 
 * @param {string} accountId - e.g. 0x145C35F41Ead9598C0F8fEe99c8bbA177C7950e9
 * @returns {Promise<{
 *   success: boolean,
  *   data: { accountvalue, totalNtLPos, totalRawUsd, totalMarginUsed } | null,
  *   error: string
  * }>}
  *   - success: True if the symbol was found and data was parsed.
  *   - data: An object with basic fields { name, leverage, price } from both arrays.
...
export async function getPerpsByName2(names = []) {
  const metaResult = await fetchHyperliquidMeta();

  if (!metaResult.success || !metaResult.universe) {
    return { success: false, universe: null, error: metaResult.error };
  }

  /* 1️⃣  No filter requested → just return everything */
  if (names.length === 0) {
    return { success: true, universe: metaResult.universe, error: null };
  }

  /* 2️⃣  Build a quick lookup table   { 'BTC': {...}, 'ETH': {...} }  */
  const lookup = Object.fromEntries(
    metaResult.universe.map((p) => [p.name.toUpperCase(), p])
  );

  /* 3️⃣  Re‑assemble the list in the same sequence that `names` came in */
  const ordered = names
    .map((n) => lookup[n.toUpperCase()])
    .filter(Boolean);                 // drop symbols that were not found

  return { success: true, universe: ordered, error: null };
}
