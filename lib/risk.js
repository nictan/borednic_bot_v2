...
`\`/market ${direction_text} ${base}/${quote} ${roundedPositionSize} ${leverage}x <TP&SL>\``, 
`-----------------------------------
*Pair DETAILS*
...
export async function periCalc(fromObj, textMsg) {
...
Please ensure your risk and SL are numbers (with or without '%' / '$').`, ...arr];
  ...
  return [`*Recommended Peri Command:*`,
`\`/market ${direction_text} ${base}/${quote} ${roundedPositionSize} ${leverage}x ${n3(tpStr)} ${n3(slStr)}\``, 
`-----------------------------------
*Pair DETAILS*
...
_You may want to include your HL Account, I can check if you got sufficient balance in your Perps account.`;
}
