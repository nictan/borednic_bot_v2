...
export async function pvpCalc(args) {
  /* replicate your previous /pvp algorithm here, return a string */
...
`\`/market ${direction_text} ${base}/${quote} ${roundedPositionSize} ${leverage}x ${tpStr} ${slStr}\``, 
`-----------------------------------
*Pair DETAILS*

\`Trading Pair:\`  ${base.toUpperCase()}/${quote.toUpperCase()}
\`Risk Amount:\`    $${riskVal.toFixed(2)}
\`Take profit:\`    ${tpStr}
\`Stop Loss:\`        ${slStr}
\`Stop Loss %:\`    ${slVal}%
...
\`Margin Used:\`       $${detailed_base_marginused}`,
`*${detailed_quote} Details:*
\`Leverage:\`              ${detailed_quote_leverage}x
\`Position Size:\`  $${half_size}
\`Margin Used:\`       $${detailed_quote_marginused}`];
}
